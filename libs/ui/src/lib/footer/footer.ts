import { ChangeDetectionStrategy, Component } from '@angular/core';

type IconKind = 'how' | 'linkedin' | 'github';

const ICON_LINKS: readonly { kind: IconKind; label: string; href: string }[] = [
  { kind: 'how', label: 'How', href: 'https://how.athreya.codes' },
  { kind: 'linkedin', label: 'LinkedIn', href: 'https://www.linkedin.com/in/athreyamr/' },
  { kind: 'github', label: 'GitHub', href: 'https://github.com/athreyacodes' },
];

@Component({
  selector: 'ui-footer',
  template: `
    <footer class="site-footer">
      <p class="credit">© {{ year }} Athreya M R</p>
      <nav class="elsewhere" aria-label="Elsewhere">
        <a
          class="home-link"
          href="https://athreya.codes/"
          target="_blank"
          rel="noopener noreferrer me"
          >athreya.codes</a
        >
        @for (link of iconLinks; track link.kind) {
          <a
            class="icon-link"
            [href]="link.href"
            target="_blank"
            rel="noopener noreferrer me"
            [attr.aria-label]="link.label"
          >
            <span class="tooltip">{{ link.label }}</span>
            @switch (link.kind) {
              @case ('how') {
                <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
                  <path
                    fill="currentColor"
                    d="M18 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2M6 4h5v8l-2.5-1.5L6 12z"
                  />
                </svg>
              }
              @case ('linkedin') {
                <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
                  <path
                    fill="currentColor"
                    d="M19 3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14m-.5 15.5v-5.3a3.26 3.26 0 0 0-3.26-3.26c-.85 0-1.84.52-2.32 1.3v-1.11h-2.79v8.37h2.79v-4.93c0-.77.62-1.4 1.39-1.4a1.4 1.4 0 0 1 1.4 1.4v4.93h2.79M6.88 8.56a1.68 1.68 0 0 0 1.68-1.68c0-.93-.75-1.69-1.68-1.69a1.69 1.69 0 0 0-1.69 1.69c0 .93.76 1.68 1.69 1.68m1.39 9.94v-8.37H5.5v8.37h2.77z"
                  />
                </svg>
              }
              @case ('github') {
                <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
                  <path
                    fill="currentColor"
                    d="M12 2A10 10 0 0 0 2 12c0 4.42 2.87 8.17 6.84 9.5.5.08.66-.23.66-.5v-1.69c-2.77.6-3.36-1.34-3.36-1.34-.46-1.16-1.11-1.47-1.11-1.47-.91-.62.07-.6.07-.6 1 .07 1.53 1.03 1.53 1.03.87 1.52 2.34 1.07 2.91.83.09-.65.35-1.09.63-1.34-2.22-.25-4.55-1.11-4.55-4.94 0-1.11.38-2 1.03-2.71-.1-.25-.45-1.29.1-2.64 0 0 .84-.27 2.75 1.02.79-.22 1.65-.33 2.5-.33s1.71.11 2.5.33c1.91-1.29 2.75-1.02 2.75-1.02.55 1.35.2 2.39.1 2.64.65.71 1.03 1.6 1.03 2.71 0 3.84-2.34 4.68-4.57 4.93.36.31.69.92.69 1.85V21c0 .27.16.59.67.5C19.14 20.16 22 16.42 22 12A10 10 0 0 0 12 2z"
                  />
                </svg>
              }
            }
          </a>
        }
      </nav>
    </footer>
  `,
  styleUrl: './footer.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class Footer {
  protected readonly year = new Date().getFullYear();
  protected readonly iconLinks = ICON_LINKS;
}
